...
export const replicateService = {
  ...
  enhanceImage: async (base64: string, mimeType: string, mode: 'polish' | 'master', originalWidth: number, originalHeight: number): Promise<string> => {
    // Create a fresh instance right before the call to use the latest selected API key
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        config: {
          imageConfig: {
            ...
            // 2K = ~2048px, 4K = ~4096px. Fulfills the user's >2000px requirement.
      ...
      // If the error is related to missing billing/project, we re-throw to trigger key selection in App.tsx
